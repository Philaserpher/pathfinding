...
def get_neighbour(pos, grid, rows):  # used to randomly select neighbours
    ...
    while True:
        ...
        key = sides[key]
        if key == 1 and pos[1] > 2 and not grid[pos[0]][pos[1] - 2].is_empty():
            grid[pos[0]][pos[1] - 1].reset()
            return (pos[0], pos[1] - 2)
        elif (
            key == 2
            and pos[0] < rows - 3
            and not grid[pos[0] + 2][pos[1]].is_empty()
        ):
            ...
            key == 3
            and pos[1] < rows - 3
            and not grid[pos[0]][pos[1] + 2].is_empty()
        ):
            grid[pos[0]][pos[1] + 1].reset()
            return (pos[0], pos[1] + 2)
        elif (
            key == 4 and pos[0] > 2 and not grid[pos[0] - 2][pos[1]].is_empty()
        ):
            grid[pos[0] - 1][pos[1]].reset()
            return (pos[0] - 2, pos[1])
        sides.remove(key)
